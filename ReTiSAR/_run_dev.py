...
def _timeit(description, stmt, setup, _globals, reference=None, check_dtype=None, repeat=10, number=2000):
    print(description)
...
result = buffer
"""
    ref = _timeit(description='numpy roll', stmt=s, setup='import numpy as np',
                  _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
...
result = buffer
"""
    _timeit(description='slice indexing', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # slower
...
"""
    ...
    _timeit(description='Ringbuffer', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
...
"""
    ...
    # this is not expected to deliver the same result as the roll methods above
    _timeit(description='Ringbuffer no copy', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
...
def _timeit_fft():
    ...
    ref = _timeit(description='numpy', stmt='result = fft.rfft(input_td)', setup='import numpy.fft as fft',
                  _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _rfft = pyfftw.builders.rfft(input_td, overwrite_input=True)
    _timeit(description='pyfftw overwrite', stmt='result = _rfft(input_td)', setup='',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _rfft = pyfftw.builders.rfft(input_td, overwrite_input=True, planner_effort='FFTW_PATIENT')
    _timeit(description='pyfftw effort', stmt='result = _rfft(input_td)', setup='',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _rfft = pyfftw.builders.rfft(input_td, overwrite_input=True, planner_effort='FFTW_PATIENT', threads=2)
    _timeit(description='pyfftw threads', stmt='result = _rfft(input_td)', setup='',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='pyfftw numpy interface', stmt='result = fft.rfft(input_td)',
            setup='import pyfftw.interfaces.numpy_fft as fft', _globals=locals(), reference=ref,
            repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='pyfftw numpy interface effort',
            stmt='result = fft.rfft(input_td, planner_effort="FFTW_PATIENT")',
            setup='import pyfftw.interfaces.numpy_fft as fft', _globals=locals(), reference=ref,
            repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='pyfftw numpy interface threads',
            stmt='result = fft.rfft(input_td, planner_effort="FFTW_PATIENT", threads=2)',
            setup='import pyfftw.interfaces.numpy_fft as fft', _globals=locals(), reference=ref,
            repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='scipy', stmt='result = fft.rfft(input_td)', setup='import scipy.fftpack as fft',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='pyfftw scipy interface', stmt='result = fft.rfft(input_td)',
            setup='import pyfftw.interfaces.scipy_fftpack as fft', _globals=locals(), reference=ref,
            repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)

    _timeit(description='mkl_fft', stmt='result = fft.rfft(input_td)', setup='import mkl_fft as fft',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
...
def _timeit_noise():
    ...
    ref = _timeit(description='noise white complex128',
                  stmt='tools.generate_noise((_CHANNEL_COUNT, _BLOCK_LENGTH), dtype="complex128")',
                  setup='from ReTiSAR import tools', _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _timeit(description='noise white complex64', reference=ref, check_dtype=np.complex64,
            stmt='tools.generate_noise((_CHANNEL_COUNT, _BLOCK_LENGTH), dtype="complex64")',
            setup='from ReTiSAR import tools', _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _timeit(description='noise white float64', reference=ref, check_dtype=np.float64,
            stmt='tools.generate_noise((_CHANNEL_COUNT, _BLOCK_LENGTH), dtype="float64")',
            setup='from ReTiSAR import tools', _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _timeit(description='noise white float32', reference=ref, check_dtype=np.float32,
...
"""
    ...
    ref = _timeit(description='AR original implementation', stmt=s, setup='import numpy as np',
                  _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _ar_coefficients = coefficients[1:]
...
result = shaped.T
"""
    _timeit(description='AR use einsum()', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # not good
...
result = shaped.T
"""
    _timeit(description='AR use inner()', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # not good
...
result = shaped.T
"""
    _timeit(description='AR use dot()', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # good (best AR)
...
result = normal.T
"""
    _timeit(description='AR use dot() 1 array', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # not good
...
result = normal.T
"""
    _timeit(description='AR use dot() 1 array iterate', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # good (best AR)
...
"""
    _ar_coefficients = coefficients[1:]
    _timeit(description='AR nditer', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # not good
...
result = normal
"""
    _timeit(description='AR nditer 1 array', stmt=s, setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # not good
...
"""
    _timeit(description='IIR with lfilter()', stmt=s, setup='from scipy import signal',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)  # good (best IIR)
...
def _timeit_sp():
    ...
    def _test_performance(d, dtype, r):
        ...
        return _timeit(description=f'numpy irfft(rfft()) as {d.dtype}',
                       stmt='np.fft.irfft(np.fft.rfft(d, axis=-1), axis=-1)',
                       setup='import numpy as np', _globals=locals(), reference=r, check_dtype=dtype,
                       repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
...
def _timeit_sht():
    ...
    spherical_harmonic_bases = tools.generate_noise((_SH_COEFFICIENTS, _CHANNEL_COUNT), dtype=np.complex128)
    ...
    def _test_performance(shb, d, dtype):
        ...
        ref = _timeit(description=f'numpy.inner() as {d.dtype}', stmt='np.inner(shb, d.T)',
                      setup='import numpy as np', _globals=locals(),
                      repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER // (2 * _SH_COEFFICIENTS))
        r1 = _timeit(description=f'numpy.einsum() as {d.dtype}', stmt='np.einsum("ij,jl", shb, d)',
                     setup='import numpy as np', _globals=locals(), reference=ref,
                     repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER // (2 * _SH_COEFFICIENTS))
        r2 = _timeit(description=f'numpy.dot() as {d.dtype}', stmt='np.dot(shb, d)',
                     setup='import numpy as np', _globals=locals(), reference=ref,
                     repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER // (2 * _SH_COEFFICIENTS))
    ...
    rs = [_test_performance(spherical_harmonic_bases, data, np.complex128),
...
def _timeit_basic():
    ...
    ref = _timeit(description='multiply', stmt='data * data', setup='',
                  _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _timeit(description='numpy.multiply()', stmt='np.multiply(data, data)', setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    print()

    ref = _timeit(description='abs', stmt='abs(data)', setup='',
                  _globals=locals(), repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
    _timeit(description='numpy.abs()', stmt='np.abs(data)', setup='import numpy as np',
            _globals=locals(), reference=ref, repeat=_TIMEIT_REPEAT, number=_TIMEIT_NUMBER)
