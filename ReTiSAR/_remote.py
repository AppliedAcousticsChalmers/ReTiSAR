...
class OscRemote(object):
    ...
    Also look at the provided PureData example showing more examples to control different kind of clients.
    ...
    def start(self, clients):
        """
        Start OSC server to receive messages for all mapped functions regarding the provided clients. In the current
        implementation running the OSC server blocks further application execution, , keeping the application alive
        until the server is released.

        Parameters
        ----------
        clients : list of SubProcess
            clients that receive a function mapping (see implementation for individual functions) so they will be
            remote controllable
        """

        def add_mapping(command, function_name, is_reversible=True):
            """
            Add mapping from an OSC target to a function. The function will be invoked with all additionally provided
            parameters as soon as the matching OSC command is received.

            Parameters
            ----------
            command : str
                OSC target command which will be used in combination with the client name to invoke the mapped function
            function_name : str
                name of the clients function that will be invoked later
            is_reversible : bool, optional
                if OSC target will be mapped twice, so the order of the command and the client name does not matter
            """
            # check if client has the named function
            if not (hasattr(client, function_name) and callable(getattr(client, function_name))):
                return
        ...
        for client in clients:
            ...
            add_mapping('volume_relative', 'set_output_volume_relative_db')  # see `JackClient`
            add_mapping('volume_port_relative', 'set_output_volume_port_relative_db')  # see `JackClient`
            add_mapping('delay', 'set_input_delay_ms')  # see `JackClient`
            ...
            # player = _setup_player(pre_renderer if (pre_renderer and pre_renderer.is_alive()) else renderer, player)
    ...
    def terminate(self):
        """
        Shutdown OSC server if it is running and in the current implementation releasing the application to continue
        execution (most likely to shutdown all child processes and terminating itself.
        """
    ...
    def handle_function(_, references, *parameters):
        """
        Call the intended function of the instance with the parameters provided. Additional parameters will be dropped,
        in case there are more given then the referenced function takes.
        ...
        references : list of SubProcess, str, logging.Logger
            instance and function that should be invoked as well as reference to logger to provide identical logging
            behaviour as calling process
        ...
        if len(parameters) == 0:
            ...
            # log_str = 'calling ... {}.{}({})'.format(type(client).__name__, function_name, *parameters)
            # logger.info(log_str) if logger else print(log_str)
