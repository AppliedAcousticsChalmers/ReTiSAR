...
def generate_noise(shape, scale=1 / 10, dtype="float64"):
    ...
    Notes
    -----
    Generation in double precision yields better performance, since single precision is not
    natively supported and has to be generated by type casting.
    """
    import numpy as np

    if np.dtype(dtype) == np.complex128:
        return np.random.normal(loc=0, scale=scale, size=(shape[0], shape[1] * 2)).view(
            np.complex128
        )
    elif np.dtype(dtype) == np.complex64:
        return (
            np.random.normal(loc=0, scale=scale, size=(shape[0], shape[1] * 2))
            .astype(np.float32)
            .view(np.complex64)
        )
