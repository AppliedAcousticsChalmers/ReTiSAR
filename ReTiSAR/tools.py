...
def parse_cmd_args():
    ...
    parser.add_argument('-irt', '--IR_TRUNCATION_LEVEL', type=float, required=False,
                        help='relative level in dB under peak to individually truncate any impulse response set after '
                             'load to save performance')
    ...
    parser.add_argument('-shp', '--SH_IS_ENFORCE_PINV', type=transform_str2bool, required=False,
                        help='if pseudo-inverse (Moore-Penrose) matrix will be used over explicitly given sampling '
                             'grid weights (only relevant for filter sets in MIRO format)')
    ...
    parser.add_argument('-hrd', '--HRIR_DELAY', type=float, required=False,
                        help='input buffer delay in ms of renderer for Head Related Impulse Responses')
    ...
    parser.add_argument('-hpl', '--HPCF_LEVEL', type=float, required=False,
                        help='output level in dBFS of renderer for Headphone Equalization Impulse Response')
    ...
    parser.add_argument('-tt', '--TRACKER_TYPE', type=str, required=False,
                        choices=['NONE', 'AUTO_ROTATE', 'POLHEMUS_PATRIOT', 'POLHEMUS_FASTRACK', 'RAZOR_AHRS'],
...
def get_pretty_delay_str(samples, fs):
    ...
    # generate string
    return f'{np.array2string(samples, separator=", ")} samples / ' \
...
def transform_str2floats(_str):
    ...
    _str = _str.replace(',', ' ').replace(';', ' ').replace('"', ' ').replace('[', ' ').replace(']', ' ') \
        .replace('{', ' ').replace('}', ' ')
...
def plot_ir_and_tf(data_td_or_fd, fs, lgd_ch_ids=None, is_label_x=True, is_share_y=True, is_draw_grid=True,
                   is_etc=False, set_td_db_y=None, set_fd_db_y=None, step_db_y=5, set_fd_f_x=None, is_draw_td=True,
                   is_draw_fd=True, is_show_blocked=None):
    """
    Parameters
    ----------
    ...
    lgd_ch_ids : array_like, optional
        IDs that should be printed in the legend as individual channel names of size [number of channels] (range from 0
    ...
    def _adjust_y_lim(is_fd=True):
        ...
        v_min, v_max = _get_y_data_lim(col) if is_share_y else axes[ch, int(col)].yaxis.get_data_interval()
    ...
    fig, axes = plt.subplots(nrows=data_td.shape[0], ncols=is_draw_td + is_draw_fd, squeeze=False,
    ...
    for ch in range(data_td.shape[0]):
        if is_draw_td:
            ...
            if length > _TD_STEM_LIM:
                axes[ch, td_col].plot(np.arange(0, length), data_td[ch], linewidth=.5, color='C0')
            else:
                axes[ch, td_col].stem(data_td[ch], linefmt='C0-', markerfmt='C0.', basefmt='C0-',
                                      use_line_collection=True)
            # set limits
            if is_etc:
                axes[ch, td_col].set_ylim(*_adjust_y_lim(is_fd=False))  # needs to be done before setting yticks
            # set ticks and grid
            axes[ch, td_col].tick_params(which='major', direction='in', top=True, bottom=True, left=True, right=True)
            ...
            if is_draw_grid:
                ...
                if length > config.BLOCK_LENGTH:
                    axes[ch, td_col].set_xticks(np.arange(0, length + 1,
                                                          length_2 // 4 if length_2 > length else length // 2),
                                                minor=False)
                    axes[ch, td_col].set_xticks(np.arange(0, length + 1, config.BLOCK_LENGTH), minor=True)
                    axes[ch, td_col].grid(True, which='both', axis='x', color='r', alpha=.4)
                else:
                    axes[ch, td_col].set_xticks(np.arange(0, length + 1, length_2 // 4 if length_2 > 4 else 2),
                                                minor=False)
                    axes[ch, td_col].grid(True, which='major', axis='x', alpha=.25)
                if is_etc:
                    axes[ch, td_col].grid(True, which='both', axis='y', alpha=.1)
                    axes[ch, td_col].set_yticks(np.arange(*axes[ch, td_col].get_ylim(), step_db_y), minor=True)
            # set limits
            if length > _TD_STEM_LIM:
                axes[ch, td_col].set_xlim(0, length)
            else:
                axes[ch, td_col].set_xticks(np.arange(0, length, 1), minor=False)  # overwrite ticks
                axes[ch, td_col].set_xlim(-.5, length - .5)
            # set labels
        ...
        if is_draw_fd:
            # # # plot spectrum # # #
            axes[ch, fd_col].semilogx(np.linspace(0, fs / 2, len(data_fd[ch])), 20 * np.log10(np.abs(data_fd[ch])),
                                      color='C1')
            # set limits
            axes[ch, fd_col].set_ylim(*_adjust_y_lim(is_fd=True))  # needs to be done before setting yticks
            # set ticks and grid
            axes[ch, fd_col].set_xticks(_FREQS_LABELED)
            axes[ch, fd_col].xaxis.set_major_formatter(FuncFormatter(lambda x, _: f'{x / 1000:.16g}'))
            axes[ch, fd_col].tick_params(which='major', direction='in', top=True, bottom=True, left=True, right=True)
            axes[ch, fd_col].tick_params(which='minor', length=0)
            if is_draw_grid:
                axes[ch, fd_col].grid(True, which='major', axis='both', alpha=.25)
                axes[ch, fd_col].grid(True, which='minor', axis='both', alpha=.1)
                axes[ch, fd_col].set_yticks(np.arange(*axes[ch, fd_col].get_ylim(), step_db_y), minor=True)
            # set limits
            axes[ch, fd_col].set_xlim(*set_fd_f_x)  # needs to be done after setting xticks
            # set labels
            if is_label_x and ch == data_td.shape[0] - 1:
