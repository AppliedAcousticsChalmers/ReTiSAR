...
def _run(logger, name, mode, filter_files, filter_file_type, block_lengths, repetitions, start_time,
    ...
    filter_files : List[str]
        list of file paths to finite impulse responses used for convolution in the generated JACK clients
    filter_file_type : str
        filter type being used for the specified finite impulse response filters, see `FilterSet.Type`
    ...
    while not finished and not interrupted:
        try:
            ...
            while len(filter_files) > 0:
                ...
                filter_len, filter_fs = _get_filter_len_and_fs(filter_file)
                ...
                while len(block_lengths) > 0:
                    block_len = block_lengths[0]

                    if mode == 'PARALLEL_CONVOLVERS':
                        # Dynamically calculate time, which this process sleeps to gather potential audio dropouts of
                        # the started clients. (Two times) either the filter length or the block size seems to be
                        # reasonable.
                        sleep_time = filter_len / filter_fs if filter_len > block_len else block_len / filter_fs
                    else:
                    ...
                    for r in range(repetitions):
                        ...
                        while not instances_is_limit:
                            ...
                            elif mode == 'PARALLEL_CONVOLVERS' and instances_n != 0:
                            ...
                            if sum(dropouts_n) == 0:
                                dropouts_n = [0]
                            results = results.append(_generate_table_entry(filter_fs, filter_len, block_len,
                                                                           instances_n, instances_is_limit,
                                                                           dropouts_n, dropouts_mean,
                                                                           load_mean),
                                                     ignore_index=True)
...
def _create_client(name, filter_file, filter_type, block_len):
    ...
    filter_file : str
        path and name of filter being loaded by the convolver
    filter_type : str
        type of filter being loaded by the convolver
    block_len : int
    ...
    # ensure system state
    r.set_output_mute(True)
    r.set_client_passthrough(False)

    return r


def _get_filter_len_and_fs(filter_file):
    """
    Parameters
    ----------
    filter_file : str
        path and name of filter impulse response file

    Returns
    -------
    tuple of int
        filter length in samples and the sampling frequency
    """
    filter_ir, filter_fs = soundfile.read(filter_file, dtype='float32')
    return filter_ir.shape[0], filter_fs
