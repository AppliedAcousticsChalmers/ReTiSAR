...
def main():
    """
    Function containing the entire benchmarking procedure including defining configuration, running the benchmark as
    well as exporting and visualizing the results.
    ...
    # ---------------------------------------
    _IS_START_OWN_JACK_SERVER = False  # if the script can be run without starting the JACK server before
    """If own `JackClient`, acting as a JACK background server to prevent it being shutdown and started with every first
    benchmarking client, should be started by this application."""
    _FILTER_LENGTHS = [128, 512, 4096, 110250]  # keep identical for comparison against old runs
    """Type of FIR filter files to run benchmarks on."""
    _BLOCK_LENGTHS = [128, 256, 512, 1024, 2048, 4096]
    """List of block lengths of the JACK audio server and clients and samples to run benchmarks on."""
    _REPETITIONS = 5
    """Number of repetitions per filter file and block length combination benchmarking run."""
    _IS_INCLUDE_OVERHEAD = False
    """If `JackRenderer` level analysis and OSC reporting overheads should be performed during benchmark."""
    # ---------------------------------------
    # END:     DEFINE BENCHMARK CONFIGURATION
    # ---------------------------------------

    # there is a limitation of JACK ports per server (256 by default), which leads to a limitation of around 64 JACK
    # clients, when each client has 2 input and 2 output ports
    ...
    results, exit_state = _run(logger=logger, name=name, mode=config.BENCHMARK_MODE, filter_lengths=_FILTER_LENGTHS,
                               block_lengths=_BLOCK_LENGTHS, repetitions=_REPETITIONS, start_time=run_time,
                               is_include_overhead=_IS_INCLUDE_OVERHEAD, jack_instances_limit=_JACK_INSTANCES_LIMIT)
...
def _run(logger, name, mode, filter_lengths, block_lengths, repetitions, start_time, is_include_overhead,
    ...
    filter_lengths : List[int]
        list of filter lengths of emulated finite impulse responses used for convolution in the generated JACK clients
    block_lengths : List[int]
        list of block lengths used for the convolutions engine (influencing the needed blocks per partitioned
        overlap-save-convolution)
    repetitions : int
        number of repetitions (being averaged afterwards) per combination of `filter_files` and `block_lengths`
    ...
    jack_instances_limit : int, optional
        maximum number of JACK client instances that can be instantiated (so far it's unclear weather this limitation
        comes from the used version of JACK or the Python API)
    ...
    while not finished and not interrupted:
        try:
                    ...
                    for r in range(repetitions):
                        ...
                        while not instances_is_limit:
                            ...
                            # instantiate client (only one in case of `Mode.PARALLEL_CONVOLVERS`
                                ...
                                instances.append(_create_client(name=f'{name}-c{len(instances):03d}',
                                                                filter_len=filter_len, block_len=block_len,
                                                                is_include_overhead=is_include_overhead))
                                # noinspection PyProtectedMember
                                fs = instances[0]._client.samplerate

                                if mode == 'PARALLEL_CONVOLVERS':
                                    # Dynamically calculate time, which this process sleeps to gather potential audio
                                    # dropouts of the started clients. (Two times) either the filter length or the block
                                    # size seems to be reasonable. This executed only after creating the client for
                                    # once, in case of 'PARALLEL_CONVOLVERS'.
                                    sleep_time = filter_len / fs if filter_len > block_len else block_len / fs
                            ...
                            if sum(dropouts_n) == 0:
                                dropouts_n = [0]
                            results = results.append(_generate_table_entry(
                                filter_fs=fs, filter_len=filter_len, block_len=block_len,
                                instances_n=instances_n, instances_is_limit=instances_is_limit,
                                dropouts_n=dropouts_n, dropouts_mean=dropouts_mean, load_mean=load_mean),
                                ignore_index=True)
            ...
            # noinspection PyUnboundLocalVariable
            logger.error(f'JACK error with block={block_len} of filter={filter_len} at fs={fs}.')
...
def _create_client(name, filter_len, block_len, is_include_overhead):
    ...
    block_len : int
        current block length in samples (determining the number of blocks for the partitioned overlap-save-convolution)
    is_include_overhead : bool
        if `JackRenderer` level analysis and OSC reporting overheads should be performed
    ...
    JackRendererBenchmark
        generated instance
    """
    r = JackRendererBenchmark(name=name, block_length=block_len, filter_length=filter_len,
                              is_single_precision=config.IS_SINGLE_PRECISION, ir_trunc_db=None,
                              is_detect_clipping=is_include_overhead, is_measure_levels=is_include_overhead,
                              is_measure_load=is_include_overhead, is_main_client=is_include_overhead,
                              is_disable_file_logger=True, is_disable_logger=True)
    ...
    # ensure system state
    r.set_output_mute(False)  # do not mute, since renderer might not convolve in that case
...
def _generate_table_entry(filter_fs, filter_len, block_len,
                          instances_n, instances_is_limit, dropouts_n, dropouts_mean, load_mean):
    """
...
def _table_highlight_limit(entry):
    """
    Format a data entry in the provided data collection. In this case an entire data row is colored red, in case the
    `instances_is_limit` attribute is true.
...
def _get_table_by_query_equal(table, column_name, value):
    """
    Return a new data set, whose elements out of the provided data set fulfill the condition: value of attribute
    `column_name` is equal to `value`.
...
def _generate_plot(results_limits, title=None):
    """
    Generate a results plot visualizing the determined number of possible JACK clients in parallel for all different
    ...
    results_limits.block_len.replace(_BLOCK_LENGTHS_PLOT, range(len(_BLOCK_LENGTHS_PLOT)), inplace=True)
    ...
    for _, group in results_grouped:
        ...
        for _, g in group.groupby('block_len'):
            ...
            x = [g.block_len.iloc[0] + (group_index + _GROUP_MARKER_OFFSET) * _GROUP_POINT_SPACING_X] \
                * len(unique_counts)  # x position per block_len including marker offset
            y = unique_counts.index - 1  # y position per unique count
            s = [10 * v for v in unique_counts.values]  # marker size per unique count with scaling
