...
class FilterSet(object):
    ...
    class Type(Enum):
        ...
        HPCF_FIR = auto()
        """
        Headphone Compensation Filter in a shape of a regular WAV file or itaAudio as being specified in the
        ITA-Toolbox.
    ...
    @staticmethod
    def create_instance_by_type(file_name, file_type, sh_max_order=None, sh_is_enforce_pinv=False):
        """
        Parameters
        ----------
        ...
        if _type is None or file_name is None:
            return None
        elif isinstance(file_name, str) and (file_name.strip('\'"') == '' or file_name.upper() == 'NONE'):
            return None
            ...
            return FilterSetMultiChannel(file_name=file_name, is_hrir=False, is_hpcf=True)
    ...
    def load(self, block_length, is_single_precision, logger=None, ir_trunc_db=None, check_fs=None,
        ...
        if not is_prevent_logging:
            ...
            tools.export_plot(figure=tools.plot_ir_and_tf(data, self._fs, set_fd_db_y=30, set_td_db_y=120, is_etc=True,
                                                          step_db_y=10), name=name, logger=logger)
    ...
    def calculate_filter_blocks_fd(self, block_length):
        ...
        if self._irs_td.dtype == np.float32:
            self._irs_blocks_fd = self._irs_blocks_fd.astype(np.complex64)  # `astype()` makes copy
...
class FilterSetMiro(FilterSet):
    ...
    def __str__(self):
        ...
        return f'[{super().__str__()[1:-1]}, _sh_max_order={self._sh_max_order}, ' \
    ...
    def calculate_filter_blocks_nm(self):
        ...
        def _calculate_filter_block_nm(block_fd):
            ...
            ir_nm = np.stack([sfa.process.spatFT_RT(data=block_fd[:, ch, :],
    ...
    def get_sh_configuration(self):
        ...
        if self._sh_max_order is None or self._sh_max_order < 0:
            raise ValueError(f'Invalid value "{self._sh_max_order}" for spherical harmonics order.')
