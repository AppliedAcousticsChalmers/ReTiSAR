...
# """Type of FIR filter file containing Array Room Impulse Responses, see `FilterSet.Type`."""
# ARIR_FILE = 'res/ARIR/RT_calib_EM32ch_struct.mat'  # Chalmers (SN 28)
# # ARIR_FILE = 'res/ARIR/RT_calib_EM32frl_struct.mat'  # Facebook Reality Labs (SN ??)
# """File with FIR filter containing Array Room Impulse Responses, see `FilterSet`."""
# ARIR_LEVEL = 0
# """Output level in dBFS of renderer for Array Room Impulse Responses, see `JackClient`."""
# SH_MAX_ORDER = 4
...
# # G_LEVEL_REL = [-0.5, -0.1, 1.3, -0.9, -0.3, 1.2, -0.3, -0.8, 0.1, -1.3, -0.5, 0.5, 1.4, -0.7, 0.5, -0.4, 0.2, 1.2,
...
# PROCESS_PRIORITY = 1
# """Process attribute, which is requested as its operation system specific priority, see `__main__`."""
...
"""MacOS specific process attribute to limit the number of open files, which is requested to be as high as the
specified value."""
...
"""Amount of effort spent during the FFTW planning stage to create the fastest possible transform, see `pyfftw`."""
PYFFTW_NUM_THREADS = int(tools.get_cpu_count() / 4)  # quarter CPU count leads to best real-time performance so far
"""Number of available FFTW threads, should be chosen carefully to not lead to oversubscription (1 is equivalent to
disabling threading in FFTW)."""
...
# noinspection PyUnboundLocalVariable,PyUnresolvedReferences
HPCF_FILE = tools.get_absolute_from_relative_package_path(HPCF_FILE) if 'HPCF_FILE' in locals() else None
# noinspection PyUnboundLocalVariable,PyUnresolvedReferences
