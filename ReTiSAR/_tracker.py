...
class HeadTracker(SubProcess):
    ...
    def run(self):
        ...
        try:
            while not self._event_terminate.is_set():
                ...
                # updating value in a process-safe manner
    ...
    def set_zero_position(self, azim_offset_deg=0.0):
        """
        ...
        Parameters
        ----------
        azim_offset_deg : float, optional
            azimuth offset in degrees, can be used to set manual head rotation when no active tracking is used
        """
        for i in HeadTracker.DataIndex:
...
class HeadTrackerSerial(HeadTracker):
    ...
    def _init_tracker(self, port):
        ...
        self._DATA_FIND_FORMAT = '[-+]?\d+\.?\d*'  # matching RegEx could also be "\s*\d+(\s*[-+]?\d*\.?\d*){6}"
        self._BAUD_RATE = 0
    ...
    def terminate(self):
        ...
        super(HeadTrackerSerial, self).terminate()
