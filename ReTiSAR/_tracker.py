...
class HeadTracker(SubProcess):
    """
    Base functionality to run data acquisition from a head tracker contained in a separate process. To run the process
    the functions `start()`, `join()` and `terminate()` have to be used.

    Attributes
    ----------
    _position_raw : multiprocessing.Array
        data array containing the absolute head position received directly from the tracker in the form of
    ...
    _position_zero : multiprocessing.Array
        data array containing the reference head position saved position reset in the form of `HeadTracker.DataIndex`,
        process-safe to be accessed by different functions of the tracker
    _position_shared : multiprocessing.Array
        data array containing the relative head position after reset in the form of `HeadTracker.DataIndex`,
        process-safe to be accessed by different subprocesses of this application as well
    """

    class Type(Enum):
        """
        Enumeration data type used to get an identification of hardware providing head tracking data to the application.
        It's attributes (with an arbitrary distinct integer value) are used as system wide unique constant identifiers.
        """
        ...
        Used by`HeadTracker` to receive head tracking data over a serial port like "/dev/tty.UC-232AC" when adapted to
        USB.
        """
        ...
        Used by`HeadTracker` to receive head tracking data over a serial port like "/dev/tty.usbserial-AH03F9XC" when
        adapted to USB.
        """

        VRPN = auto()
        """
        Generic VRPN interface to receive tracking data from a Virtual Reality Peripheral Network server application.
        ...
        AUTO_ROTATE = auto()
        """
        Generic functionality to not read tracking data from a device, but generate an automatically increasing azimuth
        angle resulting in a virtually rotating sound scene.
        """

    class DataIndex(IntEnum):
        """
        Enumeration data type used to get array indices providing storing positional tracking data in a single array.
        It's attributes (with an incrementing distinct integer value) are used as system wide unique constant
        identifiers specifying the index of the according data in the array.
        """
    ...
    def create_instance_by_type(name, tracker_type, tracker_port, *args, **kwargs):
        """
        Static method to instantiate one of the from `HeadTracker` deriving classes, depending on the given
        `HeadTracker.Type`.
        ...
        tracker_type : str or int
            direct identifier value or string containing the name of one of the provided `HeadTracker.Type` members
        tracker_port : str
            system specific path to tracker interface being provided by an appropriate hardware driver
    ...
    def print_data(array):
        """
        Static method to neatly print positional information in the provided data array with style: "[key=value, ...]".
    ...
    def _init_tracker(self, _):
        """
        Base function to initialize necessary instances for the tracker process. Deriving classes need to override this
        function to create and initialize the required attributes.
        """
        self._init_config()

    def _init_config(self):
        """
        Base function to set configuration parameters for the tracker process. Deriving classes need to override this
        function to implement the individual parameters.
        """
    ...
    def run(self):
        ...
        This implementation updates the shared position data with current values regularly and makes the process stay
        alive until the according `multiprocessing.Event` is set.
        """
            ...
            while not self._event_terminate.is_set():
                ...
                with self._position_shared.get_lock():
                    for i in HeadTracker.DataIndex:
                        self._position_shared[i] = self._position_raw[i] - self._position_zero[i]

                # communicate current value
                angles_deg = tools.transform_into_wrapped_angles(azim=self._position_shared[HeadTracker.DataIndex.AZIM],
                                                                 elev=self._position_shared[HeadTracker.DataIndex.ELEV],
                                                                 tilt=self._position_shared[HeadTracker.DataIndex.TILT],
                                                                 is_deg=True)
                if self._osc_client:
                    self._osc_client.send_message(f'{self._osc_name}/AzimElevTilt', angles_deg)
                # else:
    ...
    def _read_data(self):
        """
        Base function to read tracking data from different kinds of hardware. Deriving classes need to override this
        function to implement the actual data acquisition.
        """
        sleep(self._timeout)  # artificial timeout

    def set_zero_position(self):
        """
        Reset the virtual audio scene to be in front where current head rotation is facing. Consequently the tracker
        will always output zeros as the reference position in that exact orientation (and cartesian room coordinate).
        """
        for i in HeadTracker.DataIndex:
            self._position_zero[i] = self._position_raw[i]

        self._logger.info(f'setting head tracker zero position to {HeadTracker.print_data(self._position_zero)}.')
    ...
    def set_azimuth_position(self, azim_deg=0.0):
        ...
        azim_deg : float, optional
            azimuth position in degrees, used to manually set the head rotation in relation to the set reference
            position
        """
        self._position_raw[HeadTracker.DataIndex.AZIM] = self._position_zero[HeadTracker.DataIndex.AZIM] + azim_deg

        self._logger.info(f'setting head tracker azimuth position to [AZIM={azim_deg:.1f}].')
    ...
    def get_shared_position(self):
        ...
        multiprocessing.Array
            ...
            `HeadTracker.DataIndex`. Other subprocesses of this application can access the data via this shared address.
        """
...
class HeadTrackerRotate(HeadTracker):
    """
    Extended `HeadTracker` implementation for generating pseudo tracking data, to have a moving sound scene even without
    tracker hardware available. This is achieved by incrementing positional data, which is utilized best with a rotation
    in the horizontal plane.

    Attributes
    ----------
    _timeout : float
        virtual refresh rate of tracker, triggering a recalculation of the data again after the given wait time
    _position_step : list of float
        step size of data (see `HeadTracker.DataIndex`) being added to the current position every `_timeout`
    """

    def _init_config(self):
        """Extends the function of `HeadTracker` to configure individual parameters for the pseudo-tracker process."""
        self._timeout = 1 / 60  # data send rate
    ...
    def _read_data(self):
        """
        Extends the function of `HeadTracker` to read data from a tracker hardware, by generating pseudo tracking data
        from incrementing values at a given pace.
        """
        sleep(self._timeout)
...
class HeadTrackerSerial(HeadTracker):
    # noinspection PyUnresolvedReferences
    """
    Extended `HeadTracker` implementation for acquiring data provided via a serial port. Usually these are nowadays
    still easily accessible via USB-to-Serial-adapters, providing a virtual serial port when utilizing the correct
    driver.
    ...
    _DATA_RAW_INDEX : list of int
        positions of values (see `HeadTracker.DataIndex`) contained in the individual provided raw data string
    _DATA_RAW_SCALE : list of float
        scaling factor of values (see `HeadTracker.DataIndex`) contained in the individual provided raw data string
    _DATA_INIT_STRING : str, optional
        string sequence that has to be sent to the individual hardware after initialization to signal a continuous
        delivery of tracking data over the serial port
    _serial : serial.Serial
        bidirectional serial data connection to communicate with individual tracking hardware
    _timeout : float, optional
        timeout to keep tracker process alive in case the provided serial port is not available, but still providing
        potential functionality like manipulating the position data by the user
    """

    # noinspection PyTypeChecker
    def _init_tracker(self, port):
        """
        Extends the function of `HeadTracker` to initialize necessary instances for the tracker process, by initiating a
        bidirectional communication with a provided serial port.
        ...
        self._DATA_FIND_FORMAT = r'[-+]?\d+\.?\d*'  # matching RegEx could also be "\s*\d+(\s*[-+]?\d*\.?\d*){6}"
        self._BAUD_RATE = 0
    ...
    def _read_data(self):
        """
        Extends the function of `HeadTracker` to read data from the tracker hardware, by line-wise acquisition of byte
        strings and parsing them in the according manner.
        """
        if not self._serial:
            sleep(self._timeout)
            return
    ...
    def terminate(self):
        """
        Extends the function of `HeadTracker` to terminate the tracker process by also closing the serial data
...
class HeadTrackerPatriot(HeadTrackerSerial):
    """
    Individual implementation for Polhemus Patriot hardware, see `HeadTracker.Type.POLHEMUS_PATRIOT`.
...
class HeadTrackerFastrack(HeadTrackerSerial):
    """
    Individual implementation for Polhemus Fastrack hardware, see `HeadTracker.Type.POLHEMUS_FASTRACK`.
