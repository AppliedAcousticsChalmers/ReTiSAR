...
class JackClient(SubProcess):
    ...
    _output_volume : float
        absolute multiplier as output volume scaling, applied before writing samples into `_client.outports.get_array()`
    _output_volume_relative : numpy.ndarray
        relative individual multiplier as output volume scaling per channel, applied on top of absolute output volume
        multiplier
    _input_buffer : DelayBuffer
        buffer realizing an adjustable delay line before the input signals are delivered to internal processing
    """

    def __init__(self, name, block_length=None, output_volume_dbfs=0, input_delay_ms=0, is_single_precision=True,
                 is_main_client=True, is_detect_clipping=True, is_measure_levels=False, is_measure_load=True,
                 *args, **kwargs):
        """
        ...
        self._input_buffer = DelayBuffer(sample_rate=self._client.samplerate, block_length=block_length,
                                         delay_ms=input_delay_ms)  # needs to be done after initializing the client
        ...
        def xrun(delay):
            if not self._event_ready.is_set() or not config.IS_RUNNING.is_set() or self._output_mute:
                return
    ...
    def _client_register_inputs(self, input_count):
        ...
        self._input_buffer.init(max_length_sec=config.CLIENT_MAX_DELAY_SEC, channel_count=len(self._client.inports))
    ...
    def _client_register_outputs(self, output_count):
        ...
        self._output_volume_relative = np.ones(shape=(output_count, 1), dtype=np.float32)
    ...
    def set_input_delay_ms(self, new_delay=0.0):
        ...
        new_delay : float, optional
            new input buffer delay of the client in milliseconds, if no value is given the delay is set to 0
    ...
    def set_output_volume_db(self, value_db_fs=0.0):
        ...
        return self._set_output_volume(log_str=log_str, value_db_fs=value_db_fs, port=None)
    ...
    def set_output_volume_relative_db(self, value_db_fs=0.0):
        ...
        value_db_fs : float, None, list of float or numpy.ndarray, optional
            new relative individual output volumes of the client in Decibel_FullScale of size [number of output
            channels]

        Returns
        -------
        numpy.ndarray
            actually realized relative output volumes in Decibel_FullScale of size [number of output channels]
        """
        ...
        if value_db_fs.ndim > 1:
            self._logger.error(f'provided volume array has more than one dimension ({value_db_fs.ndim})'
                               f', "{log_str}" ignored.')
            return

        # check vector length
        if value_db_fs.size != len(self._client.outports):
            self._logger.error(f'provided volume array length ({value_db_fs.size}) does not match the number of output '
                               f'channels ({len(self._client.outports)}), "{log_str}" ignored.')
            return

        return self._set_output_volume(log_str=log_str, value_db_fs=value_db_fs[:, np.newaxis], port=None)
    ...
    def set_output_volume_port_relative_db(self, port, value_db_fs=0.0):
        ...
        if isinstance(value_db_fs, list) or isinstance(value_db_fs, np.ndarray):
            if len(value_db_fs) > 1:
                self._logger.error(f'provided volume has more than one value ({len(value_db_fs)})'
                                   f', "{log_str}" ignored.')
                return
        ...
        if not 0 <= port < len(self._client.outports):
            self._logger.error(f'provided port number is out of range, "{log_str}" ignored.')
            return

        return self._set_output_volume(log_str=log_str, value_db_fs=value_db_fs, port=port)
    ...
    def _set_output_volume(self, log_str, value_db_fs, port):
        ...
        if isinstance(port, int) and self._output_volume_relative[port] != output_volume:
        ...
        elif port is None and isinstance(output_volume, np.ndarray) and not np.equal(self._output_volume_relative,
                                                                                     output_volume).all():
            self._output_volume_relative = output_volume
            val_str = np.array2string(value_db_fs[:, 0],
                                      max_line_width=100, separator=",", precision=1, floatmode="fixed", sign="+")
            self._logger.info(f'{log_str} to {val_str} dBFS.')

        # set_output_volume_db()
        elif port is None and isinstance(output_volume, float) and self._output_volume != output_volume:
    ...
    def _process_deliver(self, output_td):
        ...
        if self._output_mute or output_td is None:
            ...
            # apply output volume (scaling for all channels) as well as relative differences (between channels)
            ...
            if self._is_measure_levels:
                ...
                if self._osc_client:
                    # OSC only works with float64, see https://github.com/attwad/python-osc/issues/102
            ...
            if self._is_first_frame:
                ...
                if (self._is_single_precision and output_td.dtype == np.float32) \
                    ...
                    self._logger.warning(f'output array dtype is unexpected: "{output_td.dtype}".')
    ...
    def _report_load(self):
        """
        Deliver via OSC (or log otherwise) individual client load based on reported JACK frame times. Furthermore
        deliver (or log) overall system load based on a prediction by JACK. Latter is is done only when configured as a
        main client.
        """
