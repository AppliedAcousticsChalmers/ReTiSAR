...
class SubProcess(mp_context.Process):
    ...
    def __init__(
        ...
        """
        Create a new instance of a separate process. According to the official documentation all
        attributes have be initialized in this `__init()__` function, to be available after
        running the spawned process.
    ...
    def _init_osc_client(self):
        ...
        self._logger.debug(
            f"sending OSC messages at ({address}, {port}, {self._osc_name}) ..."
        )
        # actual OSC messages are generated in `process()` or adjacent functions in case port was
        # opened
    ...
    def run(self):
        ...
        to run the process. This implementation generates basic logging messages and makes the
        process stay alive until the according `multiprocessing.Event` is set.
        """
    ...
    def terminate(self):
        """
        Extends the `multiprocessing.Process` function to terminate the process and generate some
        logging messages.
        """
