...
def main():
    ...
    def setup_pre_renderer():
        ...
        try:
            new_renderer = JackRenderer(name=f'{name}-PreRenderer', block_length=config.BLOCK_LENGTH,
                                        ...
                                        sh_max_order=config.SH_MAX_ORDER, sh_is_enforce_pinv=config.SH_IS_ENFORCE_PINV,
    ...
    def setup_renderer(existing_tracker, existing_pre_renderer=None, existing_generator=None):
        ...
        try:
            new_renderer = JackRenderer(name=f'{name}-Renderer', block_length=config.BLOCK_LENGTH,
                                        ...
                                        input_delay_ms=config.HRIR_DELAY, source_positions=config.SOURCE_POSITIONS,
                                        shared_tracker_data=existing_tracker.get_shared_position(),
                                        sh_max_order=config.SH_MAX_ORDER, sh_is_enforce_pinv=config.SH_IS_ENFORCE_PINV,
                                        ir_trunc_db=config.IR_TRUNCATION_LEVEL, is_main_client=True,
                                        is_measure_levels=True, is_single_precision=config.IS_SINGLE_PRECISION)
    ...
    def setup_player(existing_renderer, existing_player=None):
        ...
        new_player = None
        try:
            new_player = JackPlayer(name=f'{name}-Player', block_length=config.BLOCK_LENGTH,
                                    file_name=config.SOURCE_FILE, is_main_client=False,
                                    is_single_precision=config.IS_SINGLE_PRECISION)
    ...
    def setup_hpcf(existing_renderer):
        ...
        new_renderer = None
        try:
            new_renderer = JackRenderer(name=f'{name}-HPCF', block_length=config.BLOCK_LENGTH,
                                        filter_name=config.HPCF_FILE, filter_type=config.HPCF_TYPE,
                                        ir_trunc_db=config.IR_TRUNCATION_LEVEL, is_main_client=False,
                                        is_single_precision=config.IS_SINGLE_PRECISION)
