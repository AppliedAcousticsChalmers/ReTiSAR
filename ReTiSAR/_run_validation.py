...
def main():
    ...
    try:
        cmp_files = _parse_files_and_azimuth(path=_CMP_FILES_DIR, file_name_start='rec_Impulse_',
                                             file_name_end='_target_out.wav', azimuth_offset_deg=cmp_azimuth_offset,
                                             is_reversed=False)
    ...
    for cmp_file, cmp_azimuth in zip(*cmp_files):
        ...
        try:
            ...
            logger.info(f'... skipping comparative IR set "{os.path.relpath(cmp_file)}".')
            continue
        ...
        if (cmp_corr_n == cmp_corr_n[0]).all():
            ...
            # ir_plot = np.vstack(
            #     [np.correlate(cmp_ir[ch], ref_ir[ch], mode='valid')[np.min(cmp_corr_n) - 100:np.max(cmp_corr_n) + 100]
            #      for ch in range(len(cmp_corr_n))])
            # tools.export_plot(figure=tools.plot_ir_and_tf(ir_plot, fs=fs, is_share_y=True),
            #                   name='corr_mismatch', logger=logger)
            #
            # for ch in range(len(cmp_corr_n)):
            #     ir_plot = np.vstack([ref_ir[ch, :256], cmp_ir[ch, cmp_corr_n[ch]:cmp_corr_n[ch] + 256]])
            #     tools.export_plot(figure=tools.plot_ir_and_tf(ir_plot, fs=fs, is_share_y=True),
            #                       name=f'corr_mismatch_ch{ch}', logger=logger)

            return logger

        # align and truncate to reference IR
        cmp_ir = cmp_ir[:, cmp_corr_n:cmp_corr_n + ref_ir.shape[-1]].copy()
        logger.info(f'time aligned "{os.path.relpath(cmp_file)}" to '
                    f'{tools.get_pretty_delay_str(samples=cmp_corr_n, fs=fs)}.')
        ...
        tools.export_plot(figure=_plot_result(ref_ir=ref_ir, cmp_ir=cmp_ir, fs=fs),
                          name=name, logger=logger, file_type='pdf')
        # plot unweighted difference
        diff_tf_w = np.asarray(np.abs(np.fft.rfft(diff_ir) / np.fft.rfft(ref_ir)), dtype=np.complex_)
        tools.export_plot(figure=tools.plot_ir_and_tf(diff_tf_w, fs=fs, is_draw_td=False),
                          name=f'{name}_diff_FD_weighted', logger=logger, file_type='pdf')
        # plot weighted difference
        tools.export_plot(figure=tools.plot_ir_and_tf(np.abs(diff_ir / ref_ir), fs=fs, is_etc=True, set_td_db_y=100,
                                                      is_draw_fd=False),
                          name=f'{name}_diff_ETC_weighted', logger=logger, file_type='pdf')

        # save results
        results = results.append(_generate_table_entry(
            cmp_file_name=os.path.relpath(cmp_file), cmp_azimuth_deg=cmp_azimuth, cmp_corr_n_samples=cmp_corr_n,
            cmp_scale_dbfs=np.array2string(cmp_scale_db, precision=1, floatmode='fixed'),
            diff_rms_dbfs=np.array2string(diff_rms, precision=1, floatmode='fixed'),
            diff_max_dbfs=np.array2string(diff_max, precision=2, floatmode='fixed')), ignore_index=True)
...
def _trunc_ir(ir, cutoff_db, fs, file_name, logger):
    ...
    if n_trunc > 0:
        ...
        logger.info(f'kept "{os.path.relpath(file_name)}" at '
                    f'{tools.get_pretty_delay_str(samples=ir.shape[-1], fs=fs)}.')
...
def _plot_result(ref_ir, cmp_ir, fs, compare_frame_samples=128, is_show_fd=False):
    ...
    for ch in range(diff_ir.shape[0]):
        ...
        legend_str = ['$E_{L,ref}$', '$|E_{L,ref}-E_{L,cmp}|$', '$E_{R,ref}$', '$|E_{R,ref}-E_{R,cmp}|$']
        ...
        if is_show_fd:
            ...
            # noinspection PyUnboundLocalVariable
            axes[ch, 2].semilogx(freqs, cmp_tf[ch], linewidth=.5, alpha=.75, linestyle='--', color=color)
            # axes[ch, 2].semilogx(freqs, diff_tf[ch], linewidth=.5, color=color)

            axes[ch, 2].set_xticks(_FREQS_LABELED)
            if ch == diff_ir.shape[0] - 1:
                axes[ch, 2].xaxis.set_major_formatter(FuncFormatter(lambda xx, _: f'{xx / 1000:.16g}'))
            ...
            axes[ch, 2].tick_params(which='major', direction='in', top=True, bottom=True, left=True, right=True)
            axes[ch, 2].tick_params(which='minor', length=0)
            axes[ch, 2].grid(True, which='major', axis='both', alpha=.25)
            axes[ch, 2].grid(True, which='minor', axis='both', alpha=.1)
            axes[ch, 2].set_ylabel('Magnitude / dB')
            axes[ch, 2].set_yticks(np.arange(*axes[ch, 2].get_ylim(), _STEP_FD_Y), minor=True)
