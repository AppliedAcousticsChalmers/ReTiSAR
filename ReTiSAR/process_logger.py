...
class LoggerLevelFilter(logging.Filter):
    """
    Mimic the standard logging behaviour, so only logging entries of level `logging.WARNING` or
    above are shown in `sys.stderr`.
    """
...
def setup(subprocess_name=None, is_disable_file_logger=False):
    """
    Create a new logger instance with the given name. The logging format is set according to the
    options given in `config`. This is useful for getting convenient logging information if
    subprocesses are used in an application.

    With the help of `LoggerLevelFilter` two handlers will be implemented which mimic the default
    logging behaviour by outputting messages of level `logging.WARNING` to `sys.stderr` and
    everything below to `sys.stdout`.
    ...
    if logger.hasHandlers():
        logger.debug(
            f'created LOGGER "{logger.name}" but had inherited handlers already.'
        )
    else:
    ...
    logger.debug(
        f"initialized LOGGER [{logger.name}] at level "
...
def setup_logfile(name, ending="log"):
    """
    Prepares the infrastructure of logging to a file, in case there was a logfile name given in
    `config`. Potentially already existing files from a former session get backed up with an
    altered name (former backups get overwritten during that).
...
def _update_format_recursively(logger, fmt):
    """
    Update `logging.Formatter` style for every contained `logging.Handler` by
    `_update_format_all_handlers()` as well as the parent log. This is done in a recursive way up
    to the root log. From the root log all existing loggers are updated in the same way by
    `_update_format_for_children()`.
...
def _update_format_for_children(logger, fmt):
    """
    Update style for every existing `logging.Logger` in `logger.manager.loggerDict` (if not
    propagating).
...
def _get_format_with_extending_name(current_length):
    """
    Get logging format string as set in `config`. In case it contains a placeholder for
    self-extending name length (see `config._LOGGING_SHOW_REPLACER`), it will be replaced by the
    highest length of all logger names so far.
...
def _update_and_get_max_name_length(current_length):
    """
    Updates (or creates in case it was not existing) a parameter in `config`. It is storing the
    length of the longest logger name created so far.
