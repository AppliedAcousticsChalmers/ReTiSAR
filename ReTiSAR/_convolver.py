...
from ._filter_set import FilterSetMultiChannel, FilterSetShConfig, FilterSetSsr


class DelayBuffer(object):
    """
    Basic class to provide a delay line of time domain signals with a ringbuffer. The realized delay will always be
    integer multiples of the system audio block size. There is a system wide configurable maximum delay time.

    Attributes
    ----------
    _buffer : numpy.ndarray
        time domain block buffer of size [number of maximum delay blocks; number of input channels; `_block_length`]
    _buffer_ptr : int
        index of current block in the buffer being filled with new input, where the index of the current block in the
        buffer being delivered as output is relative to this
    _sample_rate : int
    ...
    def init(self, max_length_sec, channel_count):
        ...
        """
        # + 1 for being able to realize the maximum delay without wrapping around to the current block
        block_count = self._calculate_delay_blocks(delay_ms=max_length_sec * 1000) + 1
        self._buffer = np.zeros(shape=(block_count, channel_count, self._block_length), dtype=np.float32)
        self._reset()
...
class Convolver(object):
    ...
    def _clear_buffers(self):
        """Clear all intermediate signal block buffers, helpful to prevent artifacts when switching configurations."""
    ...
    def filter_block(self, input_td):
        """
        Process time domain samples with the given `FilterSet` by fast convolution in frequency domain.

        This implementation does NOT use any input or output buffering like overlap save or overlap add. Due to that
        it is prone to produce time aliasing artifacts. Hence, direct use this implementation is not recommended, but
        provides a foundation for the inheriting classes.
...
class OverlapSaveConvolver(Convolver):
    ...
    def _filter_block_shift_and_convert_input(self, input_block_td):
        ...
        self._input_block_td[:, :input_block_td.shape[1]] = self._input_block_td[:, input_block_td.shape[1]:]
        self._input_block_td[:, input_block_td.shape[1]:] = input_block_td

        # transform stored blocks into frequency domain
        buffer_block_fd = self._rfft(self._input_block_td) \
            if config.IS_PYFFTW_MODE else np.fft.rfft(self._input_block_td)
    ...
    def _filter_block_shift_and_convert_result(self, is_last_block=False):
        ...
        first_block_td = self._irfft(buffer_blocks_fd[0]) \
            if config.IS_PYFFTW_MODE else np.fft.irfft(buffer_blocks_fd[0])
...
class AdjustableFdConvolver(OverlapSaveConvolver):
    ...
    def filter_block(self, input_block_td):
        ...
        # transform back into time domain
        output_in_block_td = self._filter_block_shift_and_convert_result(is_last_block=False)
        ...
        # transform back into time domain
        output_out_block_td = self._filter_block_shift_and_convert_result(is_last_block=True)
...
class AdjustableShConvolver(AdjustableFdConvolver):
    ...
    _comp_arir_config : sfa.io.ArrayConfiguration
        recording / measurement microphone array configuration for spherical harmonics compensation being applied to
        the filter
    _comp_mrf_limit : int
    ...
    def prepare_sh_processing(self, input_sh_config, mrf_limit_db, compensation_type, logger=None):
        ...
        according to the provided input array configuration, as well as further compensation filters will be generated
        and applied preliminary.
    ...
    def update_sh_processing(self, sh_new_order, logger=None):
        """
        Update the current spherical harmonics processing order. With that, potentially used compensation filters
        will be re-calculated and re-applied.
        ...
        if not 0 <= sh_new_order <= self._filter._sh_max_order:
            logger.error(f'value "{sh_new_order}" out of bounds, "set SH processing order" ignored.')
            return
        if self._sh_cur_order == sh_new_order:
            return sh_new_order

        # temporarily pause execution
        event_running_state_before = config.IS_RUNNING.is_set()
    ...
    def _apply_sh_compensation(self, is_plot=True, logger=None):
        """
        Calculate and apply modal radial filter as well as further compensation filters. This function can be re-used
        during runtime, in case aspects of the rendering configuration change, i.e. the current spherical harmonics
        rendering order.
        ...
        logger : logging.Logger, optional
            instance to provide identical logging behaviour as the parent process
        """
        # reset compensation specific parameters to initial values (relevant when re-applying the compensations)
        ...
        comp_nm = Compensation.generate_by_type(compensation_types=self._comp, filter_set=self._filter,
                                                arir_config=self._comp_arir_config, amp_limit_db=self._comp_mrf_limit,
    ...
    def filter_block(self, input_block_td):
        ...
        When the current rendering order is lowered during execution, the sound field is still decomposed at the full
        maximum order. Although, only coefficients up to the specified current order are used to calculated the ear
        signals. For best efficiency also the decomposition would only be done at the current order, but the
        implementation would become much more complicated (sizes of all internal buffers in SH domain would change).
        ...
        input_block_nm = sfa.process.spatFT_RT(data=self._filter_block_shift_and_convert_input(input_block_td),
        ...
        sh_azim_nm = np.exp(self._blocks_fd.dtype.type(-1j) * self._sh_m * np.deg2rad(azim_deg))
        ...
        self._blocks_fd[0, 0] = np.sum(input_block_nm[:sh_azim_nm.shape[0]] * sh_azim_nm, axis=0)
        # transform back into time domain
        output_in_block_td = self._filter_block_shift_and_convert_result(is_last_block=False)
        ...
        self._last_blocks_fd[0, 0] = np.sum(input_block_nm[:self._last_sh_azim_nm.shape[0]] * self._last_sh_azim_nm,
                                            axis=0)
        # transform back into time domain
        output_out_block_td = self._filter_block_shift_and_convert_result(is_last_block=True)
