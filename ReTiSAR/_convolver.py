...
class Convolver(object):
    ...
    def _debug_filter_block(self, input_count, is_generate_noise=False):
        ...
        block_length = (
            ...
            if (config.IS_PYFFTW_MODE and type(self) is Convolver)
    ...
    def filter_block(self, input_td):
        ...
        input_fd = (
            self._rfft(input_td) if config.IS_PYFFTW_MODE else np.fft.rfft(input_td)
        )
        ...
        return (
            self._irfft(result_fd) if config.IS_PYFFTW_MODE else np.fft.irfft(result_fd)
        )
