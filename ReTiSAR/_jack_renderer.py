...
class JackRenderer(JackClient):
    """
    Extension of `JackClient` to provide functionality of processing audio signals in a block
    wise manner provided by the JACK client.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        filter_name : str or numpy.ndarray, optional
            file path/name of filter file or directly provided filter coefficients being used by
            `Convolver`
    ...
    def _init_convolver(
        ...
        filter_set.load(
            ...
            is_single_precision=self._is_single_precision,
            logger=self._logger,
            ir_trunc_db=ir_trunc_db,
        ...
        if type(self._convolver) == AdjustableFdConvolver and type(filter_set) in (
            ...
            self._logger.warning(
                "selection of HRIR depending on head rotation is not properly implemented yet."
            )
    ...
    def client_register_and_connect_inputs(self, source_ports=True):
        ...
        ValueError
            in case array processing will be used and the number of provided source ports is
            smaller than then array processing channels
        """
        ...
        if len(source_ports) > convolver_port_count:
            self._logger.info(
                f"tried to connect {len(source_ports)} input ports ... limited the number to "
    ...
    def _client_register_and_connect_outputs(self, target_ports=True):
        """
        Register a number of output ports according to the used `Convolver` instance to the
    ...
    def get_client_outputs(self):
        ...
        jack.Ports or list of int
            JACK client outports or list of convolver output channels, in case client was not
            started (yet)
        """
        if self.is_alive():
    ...
    def set_client_passthrough(self, new_state=None):
        ...
        new_state : bool, int, float, str or None, optional
            new passthrough state of the client, if `None` is given this function works as a toggle
            between states
    ...
    def set_client_crossfade(self, new_state=None):
        ...
        ----------
        new_state : bool, int, float, str or None, optional
            new crossfade state of the client, if `None` is given this function works as a toggle
            between states
        ...
        if new_state is True and (
            not self._convolver
            or type(self._convolver)
            not in [AdjustableFdConvolver, AdjustableShConvolver]
        ):
    ...
    def set_renderer_sh_order(self, new_order=None):
        ...
        if new_order is not None and int(new_order) != new_order:
            self._logger.error(
                f'invalid integer value "{new_order}", "set SH processing order" ignored.'
            )
            return
    ...
    def prepare_renderer_sh_processing(
        self, input_sh_config, mrf_limit_db, compensation_type
    ):
        """
        ...
        ------
        ValueError
            in case the loaded `FilterSet` and `Convolver` instances are incompatible to spherical
        ...
        if type(self._convolver) is not AdjustableShConvolver:
            ...
            raise ValueError(
                ...
                f"is incompatible for spherical harmonics processing."
            )

        self._convolver.prepare_sh_processing(
            input_sh_config, mrf_limit_db, compensation_type, self._logger
        )
...
class JackRendererBenchmark(JackRenderer):
    ...
    def _process(self, input_td):
        """
        Process block of audio data. This implementation processes additional convolvers which
        output is discarded, hence this functionality is only useful for benchmarking purposes.
